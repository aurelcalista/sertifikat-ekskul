        ...
        <div class="card-custom-admin shadow-sm">
            ...
            <form action="{{ route('admin.certificates.store') }}" method="POST" enctype="multipart/form-data" id="certForm">
                ...
                <div class="row g-3">
                    ...
                    <div class="col-12 mt-4">
                        <h5 class="fw-bold text-danger border-bottom pb-2 mb-2"><i class="fa-solid fa-image me-2"></i>4. Pengaturan Template & Berkas</h5>
                    </div>
                    ...
                    <div class="col-md-6">
                        ...
                        <select name="template_id" id="template_id" class="form-select @error('template_id') is-invalid @enderror" required>
                            @foreach($templates as $tpl)
                                <option value="{{ $tpl->id }}" data-background="{{ $tpl->background_path ? asset('storage/' . $tpl->background_path) : '' }}" {{ old('template_id') == $tpl->id || $tpl->is_default ? 'selected' : '' }}>{{ $tpl->name }} {{ $tpl->is_default ? '(Default)' : '' }}</option>
                            @endforeach
                        </select>
