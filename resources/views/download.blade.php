        ...
        <div class="d-flex justify-content-between align-items-end w-100 pt-2" style="margin-top: auto;">
            ...
            <div style="width: 40%; text-align: center;">
                <svg viewBox="0 0 100 130" style="width: 68px; height: 88px; display: inline-block; filter: drop-shadow(0px 3px 5px rgba(0,0,0,0.25));">
                    <defs>
                        <radialGradient id="gRadP_dl" cx="40%" cy="35%" r="60%">
                            <stop offset="0%" stop-color="#FFF8DC" />
                            <stop offset="30%" stop-color="#FFD700" />
                            <stop offset="60%" stop-color="#DAA520" />
                            <stop offset="100%" stop-color="#8B6508" />
                        </radialGradient>
                        <radialGradient id="gEdgP_dl" cx="50%" cy="50%" r="50%">
                            <stop offset="0%" stop-color="#FFD700" />
                            <stop offset="100%" stop-color="#8B4513" />
                        </radialGradient>
                        <linearGradient id="rLP_dl" x1="0%" y1="0%" x2="100%" y2="100%">
                            <stop offset="0%" stop-color="#1a3a7a" />
                            <stop offset="50%" stop-color="#2563EB" />
                            <stop offset="100%" stop-color="#1a3a7a" />
                        </linearGradient>
                        <linearGradient id="rRP_dl" x1="100%" y1="0%" x2="0%" y2="100%">
                            <stop offset="0%" stop-color="#1a3a7a" />
                            <stop offset="50%" stop-color="#2563EB" />
                            <stop offset="100%" stop-color="#1a3a7a" />
                        </linearGradient>
                    </defs>
                    <polygon points="42,68 30,130 42,118 50,125" fill="url(#rLP_dl)" />
                    <polygon points="58,68 70,130 58,118 50,125" fill="url(#rRP_dl)" />
                    <circle cx="50" cy="50" r="40" fill="url(#gEdgP_dl)" stroke="#8B6508" stroke-width="1" stroke-dasharray="7.5 4.5" />
                    <circle cx="50" cy="50" r="37" fill="url(#gRadP_dl)" />
                    <circle cx="50" cy="50" r="33" fill="none" stroke="#B8860B" stroke-width="1.5" />
                    <circle cx="50" cy="50" r="31" fill="url(#gRadP_dl)" />
...
document.getElementById('searchForm').addEventListener('submit', function(e) {
    ...
    .then(data => {
        ...
        if (data.success) {
            ...
            // Handle images (logos & signatures) safely
            const elLogo = document.getElementById('prevLogo');
            if (elLogo) {
                if (cert.logo_base64) {
                    elLogo.src = cert.logo_base64;
                    elLogo.style.display = 'inline-block';
                } else {
                    elLogo.src = "{{ asset('logos/logo-rakitai.png') }}";
                }
            }

            // QR Code dynamic rendering safely
            const elQr = document.getElementById('prevQrCode');
            if (elQr) {
                elQr.src = `https://api.qrserver.com/v1/create-qr-code/?size=150x150&data=${encodeURIComponent(cert.verify_url)}`;
            }

            // Set download PDF action url safely
            const elDlBtn = document.getElementById('btnDownloadPdf');
            if (elDlBtn) elDlBtn.href = cert.pdf_url;

            // Show preview modal safely
